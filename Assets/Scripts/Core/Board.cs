...
namespace Chess
{
    public class Board
    {
        ...
        {
            return allPieceLists[colourIndex * boardSize + pieceType];
        }
        ...
        public void MakeMove(Move move, bool inSearch = false)
        {
            ...
            var capturedPieceType = Piece.PieceType(Square[moveTo]);
            var movePiece = Square[moveFrom];
            var movePieceType = Piece.PieceType(movePiece);

            var moveFlag = move.MoveFlag;
            var isPromotion = move.IsPromotion;
            var isEnPassant = moveFlag == Move.Flag.EnPassantCapture;

            // Handle captures
            currentGameState |= (ushort) (capturedPieceType << boardSize);
            ...
            else
            {
                ...
                switch (moveFlag)
                {
                    case Move.Flag.EnPassantCapture:
                        var epPawnSquare = moveTo + (ColourToMove == Piece.White ? -boardSize : boardSize);
                        currentGameState |= (ushort) (Square[epPawnSquare] << boardSize); // add pawn as capture type
                    ...
                    case Move.Flag.Castling:
                        var kingside = moveTo == BoardRepresentation.g1 || moveTo == BoardRepresentation.g8;
                        var castlingRookFromIndex = kingside ? moveTo + 1 : moveTo - 2;
                        var castlingRookToIndex = kingside ? moveTo - 1 : moveTo + 1;
        ...
        public void UnmakeMove(Move move, bool inSearch = false)
        {
            ...
            var undoingWhiteMove = OpponentColour == Piece.White;
            ...
            var capturedPieceType = ((int) currentGameState >> boardSize) & 255;
            ...
            var moveFlags = move.MoveFlag;
            var isEnPassant = moveFlags == Move.Flag.EnPassantCapture;
            var isPromotion = move.IsPromotion;

            var toSquarePieceType = Piece.PieceType(Square[movedTo]);
            ...
            else if (!isPromotion) GetPieceList(movedPieceType, ColourToMoveIndex).MovePiece(movedTo, movedFrom);
            ...
            else if (isEnPassant)
            {
                ...
                var epIndex = movedTo + (ColourToMove == Piece.White ? -boardSize : boardSize);
                Square[movedTo] = 0;
                Square[epIndex] = capturedPiece;
            ...
            else if (moveFlags == Move.Flag.Castling)
            {
                ...
                var kingside = movedTo == 6 || movedTo == 254;
            ...
            if (!inSearch && RepetitionPositionHistory.Count > 0) RepetitionPositionHistory.Pop();
        }
        ...
        public void LoadPosition(string fen)
        {
            ...
            for (var squareIndex = 0; squareIndex < 256; squareIndex++)
            {
                var piece = loadedPosition.squares[squareIndex];
                Square[squareIndex] = piece;

                if (piece != Piece.None)
                {
                    var pieceType = Piece.PieceType(piece);
                    var pieceColourIndex = Piece.IsColour(piece, Piece.White) ? WhiteIndex : BlackIndex;
            ...
            var epState = loadedPosition.epFile << 4;
